...
def get_dns(domain: str) -> dict:
    ...
    for rtype in ["A", "MX", "NS", "TXT", "AAAA", "CNAME"]:
        try:
            records[rtype] = [str(r) for r in dns.resolver.resolve(domain, rtype, lifetime=5)]
        except dns.resolver.NoAnswer:
            records[rtype] = "No record"
        except dns.resolver.NXDOMAIN:
            records[rtype] = "Domain does not exist"
        except Exception:
            records[rtype] = "Lookup failed"
    return records
...
def check_vulnerabilities(scan_data: dict) -> list:
    ...
    def add(vuln_id: str, evidence: str = ""):
...
def save_pdf(domain: str, data: dict) -> str:
    ...
    el += [Paragraph(
        ...
        f"HTTP headers, SSL/TLS, open ports, website metadata and cookie security. "
    ...
    el += [
        ...
        Spacer(1, 8),
        Paragraph(
            f"Generated by CLI Recon Tool  ·  For authorised security assessment use only  ·  {datetime.now().strftime('%d %b %Y')}",
            S["footer"]
        ),
    ]
    doc.build(el)
...
def main():
    ...
    print(Fore.CYAN + "\n[+] Running recon tasks in parallel...\n")
    ...
    try:
        ...
        logging.info(f"Scanned: {domain} | IP: {ip} | Vulns: {len(results.get('Vulnerabilities',[]))} | Report: {pdf_file}")
